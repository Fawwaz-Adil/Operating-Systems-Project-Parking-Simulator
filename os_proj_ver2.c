...
#define COL_PURPLE      CLITERAL(Color){180,  82, 255, 255}
...
typedef enum { TYPE_BIKE = 0, TYPE_CAR, TYPE_HEAVY, TYPE_COUNT } VehicleType;
typedef enum { PRIO_NORMAL = 0, PRIO_VIP, PRIO_EMERGENCY, PRIO_COUNT } VehiclePriority;
...
typedef struct {
    ...
    // Custom Priority Allocator (Replaces Semaphores)
    pthread_mutex_t    alloc_mutex;
    pthread_cond_t     alloc_cond[TYPE_COUNT][PRIO_COUNT]; 
    int                wait_counts[TYPE_COUNT][PRIO_COUNT];
    ...
    // Performance Metrics
    int                vehicles_served;
    int                served_by_prio[PRIO_COUNT];
    double             total_wait_time_ms;
    double             utilization_sum;
    int                utilization_ticks;
...
typedef struct { 
    ...
    VehiclePriority priority;
    ParkingLot *lot; 
    int target_slot;  // -1 for auto-assign based on priority
    int auto_leave;   // 0 for manual, >0 for seconds to wait before leaving
} VehicleArg;
...
static void draw_vehicle(int sx, int sy, int sw, int sh, float anim, int vid, VehicleType type, VehiclePriority prio);
...
static void spawn_vehicle(ParkingLot *lot, VehicleType type, VehiclePriority prio, int slot_idx, int auto_leave);

// Priority Allocation Logic
static int get_free_slot(ParkingLot *lot, VehicleType type) {
    for (int i = 0; i < TOTAL_SLOTS; i++) {
        if (lot->slots[i].allowed_type == type && lot->slots[i].state == SLOT_FREE) return i;
    }
    return -1;
}

static int wait_and_lock_slot(ParkingLot *lot, VehicleType type, VehiclePriority prio) {
    pthread_mutex_lock(&lot->alloc_mutex);
    int slot = -1;
    while ((slot = get_free_slot(lot, type)) == -1) {
        lot->wait_counts[type][prio]++;
        pthread_cond_wait(&lot->alloc_cond[type][prio], &lot->alloc_mutex);
        lot->wait_counts[type][prio]--;
    }
    lot->slots[slot].state = SLOT_RESERVED; // Lock it to prevent others from stealing
    pthread_mutex_unlock(&lot->alloc_mutex);
    return slot;
}

static void release_slot(ParkingLot *lot, int idx) {
    pthread_mutex_lock(&lot->alloc_mutex);
    VehicleType type = lot->slots[idx].allowed_type;
    lot->slots[idx].state = SLOT_FREE;
    
    // WAKE UP BY PRIORITY: Emergency -> VIP -> Normal
    if (lot->wait_counts[type][PRIO_EMERGENCY] > 0) {
        pthread_cond_signal(&lot->alloc_cond[type][PRIO_EMERGENCY]);
    } else if (lot->wait_counts[type][PRIO_VIP] > 0) {
        pthread_cond_signal(&lot->alloc_cond[type][PRIO_VIP]);
    } else if (lot->wait_counts[type][PRIO_NORMAL] > 0) {
        pthread_cond_signal(&lot->alloc_cond[type][PRIO_NORMAL]);
    }
    pthread_mutex_unlock(&lot->alloc_mutex);
}
...
int main(void)
{
    ...
    InitWindow(WIN_W, WIN_H, "OS Synchronization - Priority Parking");
    SetTargetFPS(FPS);
    ...
    while (!WindowShouldClose()) {
        ...
        // Metric Sampling (every 1 second)
        if (timer >= 1.0f) {
            pthread_mutex_lock(&lot->stats_mutex);
            lot->utilization_sum += (double)lot->occupied_count / TOTAL_SLOTS;
            lot->utilization_ticks++;
                ...
                if (CheckCollisionPointRec(mouse, lot->slots[i].bounds)) {
                    if (lot->ui_state >= UI_ADD_BIKE && lot->ui_state <= UI_ADD_HEAVY) {
                        ...
                        if (lot->slots[i].state == SLOT_FREE && lot->slots[i].allowed_type == req_type) {
                            ...
                            // To bypass the queue for manual clicks, we lock the allocator mutex, 
                            // manually mark reserved, and spawn the thread with a fixed slot.
                            pthread_mutex_lock(&lot->alloc_mutex);
                            lot->slots[i].state = SLOT_RESERVED;
                            pthread_mutex_unlock(&lot->alloc_mutex);

                            spawn_vehicle(lot, req_type, PRIO_NORMAL, i, 0);
...
static void *test_runner_thread(void *arg)
{
    ...
    push_event(lot, COL_PURPLE, "=== STRESS TEST INITIALIZED ===");
    
    // 1. Fill up all car slots to force a queue
    for(int i=0; i<NUM_CAR_SLOTS; i++) {
        spawn_vehicle(lot, TYPE_CAR, PRIO_NORMAL, -1, 6 + (rand()%3)); // Auto leave in 6-8s
        usleep(100000);
    }
    
    usleep(1000000); // Wait 1 sec
    
    // 2. Spawn a mix of vehicles that will hit the waiting queue
    push_event(lot, COL_WHITE, "[TEST] Spawning queued vehicles...");
    
    // Normal vehicles
    spawn_vehicle(lot, TYPE_CAR, PRIO_NORMAL, -1, 4);
    spawn_vehicle(lot, TYPE_CAR, PRIO_NORMAL, -1, 5);
    usleep(50000);
    
    // VIP vehicle
    spawn_vehicle(lot, TYPE_CAR, PRIO_VIP, -1, 4);
    usleep(50000);
    
    // Emergency vehicle (Should jump to the absolute front of the line)
    spawn_vehicle(lot, TYPE_CAR, PRIO_EMERGENCY, -1, 3);
...
static void *vehicle_thread(void *arg)
{
    ...
    VehiclePriority prio = va->priority;
    int idx = va->target_slot;
    int auto_leave = va->auto_leave;
    free(va);
    pthread_detach(pthread_self()); 

    const char* t_str = (type == TYPE_BIKE) ? "Bike" : (type == TYPE_CAR) ? "Car" : "Heavy";
    const char* p_str = (prio == PRIO_EMERGENCY) ? "[EMERG]" : (prio == PRIO_VIP) ? "[VIP]" : "[NORM]";
    Color p_col = (prio == PRIO_EMERGENCY) ? COL_RED : (prio == PRIO_VIP) ? COL_PURPLE : COL_MUTED;

    struct timeval spawn_time, assign_time;
    gettimeofday(&spawn_time, NULL);

    push_event(lot, p_col, "%s %s V%02d arrived. Waiting for slot...", p_str, t_str, vid);

    // PRIORITY ALLOCATION (Wait if no slots available)
    if (idx == -1) {
        idx = wait_and_lock_slot(lot, type, prio);
    }

    gettimeofday(&assign_time, NULL);
    double wait_ms = (assign_time.tv_sec - spawn_time.tv_sec)*1000.0 + (assign_time.tv_usec - spawn_time.tv_usec)/1000.0;

    pthread_mutex_lock(&lot->stats_mutex);
    lot->total_wait_time_ms += wait_ms;
    pthread_mutex_unlock(&lot->stats_mutex);
    ...
    push_event(lot, COL_GREEN, "%s %s V%02d ASSIGNED → Slot %02d (Wait: %.1fms)", p_str, t_str, vid, sid, wait_ms);
    enqueue_log(lot, "[%s] %s V%02d ENTERED slot %02d (Wait: %.1fms)", ts(), t_str, vid, sid, wait_ms);

    // Wait condition: Auto-leave timer OR manual GUI click
    if (auto_leave > 0) {
        sleep(auto_leave);
    } else {
    ...
    pthread_mutex_lock(&lot->stats_mutex);
    lot->vehicles_served++;
    lot->served_by_prio[prio]++;
    pthread_mutex_unlock(&lot->stats_mutex);

    // Trigger allocation wakeup for waiting vehicles
    release_slot(lot, idx);
    
    push_event(lot, COL_AMBER, "%s %s V%02d REMOVED ← Slot %02d", p_str, t_str, vid, sid);
...
static void *logger_thread(void *arg)
{
    ...
    if (!fp) return NULL;
    fprintf(fp, "Interactive Priority Parking Log\n\n");
...
static void draw_sidebar(ParkingLot *lot, Font font)
{
    ...
    DrawText("PRIORITY OS PARKING", x, y, 16, COL_ACCENT); y += 30;

    // --- GUI BUTTONS ---
    DrawText("1. ADD VEHICLES (NORMAL)", x, y, 12, COL_WHITE); y += 20;
    ...
    DrawText("3. AUTOMATED LOAD TEST", x, y, 12, COL_WHITE); y += 20;
    if (gui_button((Rectangle){x, y, 210, 30}, "RUN STRESS TEST", COL_PURPLE)) {
        pthread_t tid;
        pthread_create(&tid, NULL, test_runner_thread, lot);
    }
    y += 45;
...
static void draw_slot(int sx, int sy, int sw, int sh, ParkingSlot *s)
{
    ...
    if (s->state == SLOT_OCCUPIED || s->anim > 0.0f) {
        draw_vehicle(sx, sy, sw, sh, s->anim, s->vehicle_id, s->allowed_type, s->priority);
...
static void draw_vehicle(int sx, int sy, int sw, int sh, float anim, int vid, VehicleType type, VehiclePriority prio)
{
    ...
    // Color by priority to make tests easy to visualize
    Color body = (prio == PRIO_EMERGENCY) ? COL_RED : (prio == PRIO_VIP) ? COL_PURPLE : ColorFromHSV((float)((vid * 47) % 360), 0.6f, 0.8f);
...
static void draw_stats(ParkingLot *lot, Font font)
{
    ...
    DrawText("LIVE PERFORMANCE METRICS", x, y, 14, COL_WHITE); y += 28;
    ...
    // Averages
    double avg_wait = lot->vehicles_served > 0 ? (lot->total_wait_time_ms / lot->vehicles_served) : 0.0;
    double avg_util = lot->utilization_ticks > 0 ? (lot->utilization_sum / lot->utilization_ticks) * 100.0 : 0.0;

    DrawText("AVG QUEUE WAIT TIME", x, y, 11, COL_MUTED); y += 18;
    char wt[32]; snprintf(wt, sizeof(wt), "%.2f ms", avg_wait);
    DrawText(wt, x, y, 16, COL_AMBER); y += 30;

    DrawText("AVG SLOT UTILIZATION", x, y, 11, COL_MUTED); y += 18;
    char ut[32]; snprintf(ut, sizeof(ut), "%.1f %%", avg_util);
    DrawText(ut, x, y, 16, COL_BLUE); y += 30;
    DrawLine(x, y, STATS_X + STATS_W - 16, y, COL_BORDER); y += 14;

    // Fairness Control
    DrawText("THREAD FAIRNESS (SERVED)", x, y, 11, COL_MUTED); y += 20;
    
    char n_str[16], v_str[16], e_str[16];
    snprintf(n_str, sizeof(n_str), "%d", lot->served_by_prio[PRIO_NORMAL]);
    snprintf(v_str, sizeof(v_str), "%d", lot->served_by_prio[PRIO_VIP]);
    snprintf(e_str, sizeof(e_str), "%d", lot->served_by_prio[PRIO_EMERGENCY]);

    DrawText("NORMAL:", x, y, 11, COL_WHITE); DrawText(n_str, x + 100, y, 12, COL_MUTED); y += 20;
    DrawText("VIP:", x, y, 11, COL_WHITE); DrawText(v_str, x + 100, y, 12, COL_PURPLE); y += 20;
    DrawText("EMERGENCY:", x, y, 11, COL_WHITE); DrawText(e_str, x + 100, y, 12, COL_RED); y += 20;
    
    DrawLine(x, y, STATS_X + STATS_W - 16, y, COL_BORDER); y += 14;

    DrawText("TOTAL SERVED", x, y, 11, COL_MUTED);
...
static void lot_init(ParkingLot *lot)
{
    ...
    pthread_mutex_init(&lot->alloc_mutex, NULL);
    for(int t=0; t<TYPE_COUNT; t++) {
        for(int p=0; p<PRIO_COUNT; p++) {
            pthread_cond_init(&lot->alloc_cond[t][p], NULL);
            lot->wait_counts[t][p] = 0;
        }
    }
    ...
    memset(lot->served_by_prio, 0, sizeof(lot->served_by_prio));
    lot->total_wait_time_ms = 0;
    lot->utilization_sum = 0;
    lot->utilization_ticks = 0;
...
static void lot_destroy(ParkingLot *lot)
{
    ...
    for(int t=0; t<TYPE_COUNT; t++) {
        for(int p=0; p<PRIO_COUNT; p++) {
            pthread_cond_destroy(&lot->alloc_cond[t][p]);
        }
    }
    pthread_mutex_destroy(&lot->alloc_mutex);
